...
LetвЂ™s say that we need to draw a **rounded** corner background on text, supporting the following cases:
...
In [previous articles](https://medium.com/google-developers/underspanding-spans-1b91008b97e4) weвЂ™ve covered styling sections of text (even [internationalized text](https://medium.com/google-developers/styling-internationalized-text-in-android-f99759fb7b8f)). The solution involved using either framework or custom spans. While spans are a great solution in many cases, they do have some limitations that make them unsuitable for our problem. Appearance spans, like `BackgroundColorSpan`, give us access to the `TextPaint`, allowing us to change elements like the background color of text, but are only able to draw a solid color and canвЂ™t control elements like the corner radius.
...
Spans work at the `TextPaint` level, not the layout level. Therefore, they canвЂ™t know about the line number the text starts and ends on, or about the paragraph direction (left-to-right or right-to-left)
...
* Providing the background **drawables** as **attributes** of the TextView вЂ” implemented in `TextRoundedBgAttributeReader`
* **Rendering the drawables** depending on whether the text runs across **one or multiple lines** вЂ” `SearchBgRenderer` interface and its implementations: `SingleLineRenderer` and `MultiLineRenderer`
* Supporting **custom drawing** on a `TextView` вЂ” `RoundedBgTextView`, a class that extends `AppCompatTextView`, reads the attributes with the help of `TextRoundedBgAttributeReader`, overrides onDraw where it uses `SearchBgHelper` to draw the background.
...
**Caveat:** Our sample makes all the calculations for every `TextView.onDraw` method call. If youвЂ™re planning on integrating this implementation in your app, we strongly suggest to modify it and cache the calculations done in `SearchBgHelper`. Then, make sure you invalidate the cache whenever anything related to text, like text color, size or other properties, has changed.
