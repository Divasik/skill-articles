...
interface CategoriesDao : BaseDao<Category> {
    ...
    @Query("""
        SELECT category.title AS title, category.icon, category.category_id AS category_id, COUNT(category.category_id) as articles_count
        FROM article_categories AS category
        INNER JOIN articles AS article ON category.category_id = article.category_id
        GROUP BY category.category_id
        ORDER BY articles_count DESC
    """)
    fun findAllCetegoriesData(): LiveData<List<CategoryData>>
