...
class SearchBgHelper(
    ...
    private val singleLineRender: SingleLineRender= SingleLineRender(padding, drawable)
    private val multiLineRender: MultiLineRender = MultiLineRender(
            padding,
            drawableLeft,
            drawableMiddle,
            drawableRight
        ...
        spans.forEach {
            ...
            if(headerSpans.isNotEmpty()) {
                topExtraPadding = if(spanStart in headerSpans[0].firstLineBounds
                        || spanEnd in headerSpans[0].firstLineBounds) headerSpans[0].topExtraPadding else 0
                bottomExtraPadding = if(spanStart in headerSpans[0].lastLineBounds
                        || spanEnd in headerSpans[0].lastLineBounds) headerSpans[0].bottomExtraPadding else 0
            }
...
abstract class SearchBgRender(
    ...
    abstract fun draw(
            ...
            startOffset: Int,
            endOffset: Int,
            topExtrasPadding: Int = 0,
            bottomExtrasPadding: Int = 0
    )
...
class SingleLineRender(
    ...
    override fun draw(canvas: Canvas, layout: Layout, startLine: Int, endLine: Int, startOffset: Int, endOffset: Int, topExtrasPadding: Int, bottomExtrasPadding: Int) {
        lineTop = getLineTop(layout, startLine) + topExtrasPadding
        lineBottom = getLineBottom(layout, startLine) - bottomExtrasPadding
...
class MultiLineRender(
    ...
    override fun draw(canvas: Canvas, layout: Layout, startLine: Int, endLine: Int, startOffset: Int, endOffset: Int, topExtrasPadding: Int, bottomExtrasPadding: Int) {
        ...
        lineBottom = getLineBottom(layout, endLine) - bottomExtrasPadding
