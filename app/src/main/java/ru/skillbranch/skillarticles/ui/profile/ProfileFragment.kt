...
class ProfileFragment() : BaseFragment<ProfileViewModel>() {
    ...
    override val binding by lazy { ProfileBinding() }
                ...
                AvatarActionsDialog.EDIT_KEY -> {
                    lifecycleScope.launch(Dispatchers.IO) {
                        val sourceFile = Glide.with(requireActivity())
                                .asFile().load(binding.avatar).submit().get()
    ...
    private fun callbackGallery(result: Uri?) {
        if (result != null) {
            val inputStream = requireActivity().contentResolver.openInputStream(result)
            viewModel.handleUploadPhoto(inputStream)
        }
    }
